            ...
            {{-- <div class="flex flex-col flex-wrap mb-4  mt-4 space-y-4 md:flex-row md:items-end md:space-x-4 justify-end">
                <div>
                    <button
                        ...
                        <a href="{{ url('add-stock_out') }}">add stock_out
                        </a>
                    </button>
                </div>
            </div> --}}
                        ...
                        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            @foreach ($stock_out as $item)
                                <tr class="text-gray-700 dark:text-gray-400">
                                    ...
                                    <td class="px-4 py-3 text-sm">{{ $item->request_name }}</td>

                                    <td class="px-4 py-3 text-sm">{{ $item->count }}</td>
                                    <td class="px-4 py-3 text-sm ">{{ $item->model }}</td>
                                    <td class="px-4 py-3 text-sm ">{{ $item->serial }}</td>
                                    <td class="px-4 py-3">
                                        <div class="flex items-center space-x-4 text-sm">
                                            <a href="{{ url('edit-stock_out/' . $item->id) }}">
                                                <button
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center space-x-4 text-sm">
                                            <a href="{{ url('delete-stock_out/' . $item->id) }}">
                                                <button
                                                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                                    aria-label="Delete">
                                                    <svg class="w-5 h-5" aria-hidden="true" fill="currentColor"
