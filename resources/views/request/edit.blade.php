     ...
     <main class="h-full pb-16 overflow-y-auto">
         <div class="container grid px-6 mx-auto">
             ...
             <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                 <!-- Invalid input -->
                 <form action="{{ url('update-request/' . $req->id) }}" method="POST" enctype="multipart/form-data">
                     @csrf
                     ...
                     <select
                         ...
                         @foreach ($user as $name)
                             {{-- <option> {{ $req->user_name }}</option> --}}
                             <option value="{{ $name->name }} " {{ $name->name == $req->user_name ? 'selected' : '' }}>
                                 {{ $name->name }}</option>
                         @endforeach
                     ...
                     <select
                         ...
                         @foreach ($stock as $name)
                             <option value="{{ $name->product_name }}"
                                 {{ $name->product_name == $req->stock_name ? 'selected' : '' }}>{{ $name->product_name }}
                             </option>
                         @endforeach
                     ...
                     <input type="number" name="count" value="{{ $req->count }}"
                         class="  block w-full mt-1 text-sm border-green-600 dark:text-gray-300 dark:bg-gray-700 focus:border-green-400 focus:outline-none focus:shadow-outline-green form-input" />
                     ...
                     <select
                         ...
                         @error('head') is-invalid @enderror" name="head" id="head" ">
                                     ...
                                     {{-- <option selected disabled>-- Select head type --</option> --}}
                                        @foreach ($user as $name)
                         <option value="{{ $name->name }}" {{ $name->name == $req->head ? 'selected' : '' }}>
                             {{ $name->name }}
                         </option>
                         @endforeach
                     ...
                     <select
                         ...
                         @foreach ($user as $name)
                             <option value="{{ $name->name }}"
                                 {{ $name->name == $req->store_manager ? 'selected' : '' }}>{{ $name->name }}</option>
                         @endforeach
